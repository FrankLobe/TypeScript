...
class EmploymentPage {
    ...
    static getEmploymentPoints(employmentPoints) {
        return employmentPoints.map(employmentPoint => `<li class="list-group-item">${employmentPoint.point_content}</li>`).join('');
    }
    static getFormattedContent(employment) {
...
EmploymentPage.initializePage().then(() => {
    const employmentContent = document.getElementById('employment');
